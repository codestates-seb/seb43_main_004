...
public interface MealRepository extends JpaRepository<Meal, Long> { //뽑아서 day에 넣어준다는거, 각각 매핑해서 엔티티에 넣음
    ...
    @Query("SELECT new com.mainproject.wrieating.meal.entity.Week(SUM(m.kcal), AVG(m.kcal), AVG(m.carbohydrate), AVG(m.protein), AVG(m.fat), AVG(m.sugar), AVG(m.salt)) FROM Meal m WHERE m.diary.diaryId = :diaryId")
    Week getMealAverageByDiaryId(@Param("diaryId") Long diaryId);
}